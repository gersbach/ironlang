...
use crate::{
    ...
    AssignmentExpressionSyntax, BinaryExpressionSyntax, NameExpressionSyntax,
    ParenthesizedExpression, SyntaxKind, SyntaxNode, UnaryExpressionSyntax, Value,
};
...
impl<'a> Binder<'a> {
    ...
    pub fn bind_expression(&mut self, expression_kind: SyntaxNode) -> BoundNodeKind {
        return match expression_kind {
            ...
            SyntaxNode::UnaryExpressionSyntax(unary_node) => {
                BoundNodeKind::BoundUnaryExpressionNode(self.bind_unary_expresssion(unary_node))
            }
            SyntaxNode::BinaryExpressionSyntax(binary) => {
                BoundNodeKind::BoundBinaryExpression(self.bind_binary_expression(binary))
            }
    ...
    pub fn bind_unary_expresssion(
        ...
        return BoundUnaryExpressionNode {
            operator_kind: Box::from(bound_operator_kind),
    ...
    pub fn bind_binary_expression(
        ...
        return BoundBinaryExpression {
            left: Box::from(left),
            bound_binary_operator_kind: Box::from(bound_operand),
